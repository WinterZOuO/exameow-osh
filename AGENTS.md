...
Architecture reference for AI agents working on **ExamBot**. Read this before exploring the codebase.
...
- **Shared core logic**: Rust crate `exambot-core` (parsing, AI client, exam gen, export, encrypted config)
...
packages/
  core/                Rust crate `exambot-core` (shared server logic)
  ...
  shared/              TS shared types (@exambot/shared) src/types.ts
...
| Axum server | `cargo run -p exambot-server` (port 3000) |
...
- **Browser `localStorage`**: question banks, practice sessions, wrong questions, config. Keys: `exambot-banks`, `exambot-practice-session`, `exambot-wrong`, `exambot-questions`, `exambot-sourcefile`.
- **Native (Tauri/Axum)**: AI credentials encrypted with AES-256-GCM via `ConfigStore` in OS config dir (macOS `~/Library/Application Support/ExamBot/`, Linux `~/.config/ExamBot/`, Windows `%APPDATA%/ExamBot/`).
...
- CI releases on `v*` tags: Tauri (linux/windows/macOS, x86_64+aarch64) + Docker to `ghcr.io/heshengtao/exambot`.
