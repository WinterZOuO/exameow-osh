      ...
      <div class="card-outlined p-5">
        ...
        <div class="flex items-center gap-4">
          <div class="relative w-28 h-28 shrink-0">
            <svg viewBox="0 0 128 128" class="w-full h-full -rotate-90">
              <circle cx="64" cy="64" :r="PIE_R" fill="none" stroke-width="20" style="stroke: rgba(var(--md-primary) / 0.08)" />
              <circle
          ...
          <div class="flex-1 space-y-1.5 min-w-0">
            <div v-for="seg in typeSegments" :key="seg.type" class="flex items-center gap-2 text-sm">
              ...
              <span class="flex-1 truncate">{{ typeLabel(seg.type) }}</span>
              <span class="tabular-nums" style="color: rgb(var(--md-on-surface-variant))">{{ seg.count }}</span>
            </div>
          </div>
        </div>
      </div>
