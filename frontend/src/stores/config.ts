...
import type { AIConfig, ModelInfo } from '@quizseek/shared'
import { api } from '@/api'
...
export const useConfigStore = defineStore('config', () => {
  ...
  async function loadSaved() {
    ...
    if (isCloudflare()) {
      const provider = localStorage.getItem('quizseek_ai_provider')
  ...
  async function save() {
    localStorage.setItem('quizseek_ai_provider', aiProvider.value)
